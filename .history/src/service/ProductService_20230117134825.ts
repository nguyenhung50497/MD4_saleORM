...
class ProductService {
    ...
    priceRange1 = async (value) => {
        ...
        switch (value) {
            case 99:
                products = await this.productRepository.find({ $and: [{price: {$gte: 0}}, {price: {$lte: value}}]}).populate('category');
                if (!products) {
                    return null;
                }
                return products;
            case 499:
                products = await this.productRepository.find({ $and: [{price: {$gte: 100}}, {price: {$lte: value}}]}).populate('category');
                if (!products) {
                    return null;
                }
                return products;
            case 999:
                products = await this.productRepository.find({ $and: [{price: {$gte: 500}}, {price: {$lte: value}}]}).populate('category');
                if (!products) {
                    return null;
                }
                return products;
            case 1999:
                products = await this.productRepository.find({ $and: [{price: {$gte: 1000}}, {price: {$lte: value}}]}).populate('category');
                if (!products) {
                    return null;
                }
                return products;
            default:
                return products = await this.productRepository.find().populate('category');
        }
    }
}
